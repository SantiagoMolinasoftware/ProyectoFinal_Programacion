...
// Constantes globales
#define HORAS_SEMANALES 40 // Número de horas semanales de trabajo
#define DIAS_MES 30 // Número de días en un mes
...
typedef struct Empleado {
    ...
    double salarioMensualUSD; // Salario mensual en dólares
    ...
    int trabajoFeriadoBooleano; // Indicador de si ha trabajado en feriados (1 para sí, 0 para no)
    int horasExtrasDiurnas; // Número de horas extras diurnas trabajadas en el mes
} Empleado;
...
int calcular_horas_trabajadas(int horaInicio, int minutoInicio, int horaFin, int minutoFin, int tiempoDescanso); // Calcula el número de horas trabajadas en el mes
    ...
    do {
        ...
        switch (opcion) {
            ...
            case 3:
                ...
                int confirmacion; // Variable para almacenar la confirmación del usuario
...
// Función para solicitar los datos de un empleado por teclado
void solicitar_datos(Empleado *empleado) {
    printf("Ingrese el nombre del empleado: ");
    scanf(" %[^\n]", empleado->nombreCompleto); // Leer el nombre completo hasta el final de la línea
    ...
    validar_entrada(&empleado->horaInicioDiurna, 0, 23, "La hora de inicio debe estar entre 0 y 23."); // Validar que la hora de inicio sea válida
    validar_entrada(&empleado->minutoInicioDiurno, 0, 59, "El minuto de inicio debe estar entre 0 y 59."); // Validar que el minuto de inicio sea válido
    printf("Ingrese la hora de fin de labores (hh:mm): ");
    scanf("%d:%d", &empleado->horaFinDiurna, &empleado->minutoFinDiurno);
    validar_entrada(&empleado->horaFinDiurna, 0, 23, "La hora de fin debe estar entre 0 y 23."); // Validar que la hora de fin sea válida
    ...
    validar_entrada(&empleado->trabajoFeriadoBooleano, 0, 1, "El indicador de feriado debe ser 0 o 1."); // Validar que el indicador de feriado sea válido
    printf("Ha trabajado horas extras? (Ingrese el numero de horas extras): ");
    scanf("%d", &empleado->horasExtrasDiurnas);
    validar_entrada(&empleado->horasExtrasDiurnas, 0, 24, "El número de horas extras debe estar entre 0 y 24."); // Validar que el número de horas extras sea válido
}

// Función para guardar los datos de un empleado en un archivo
...
// Función para mostrar el sueldo mensual de los empleados registrados en el archivo
void mostrar_sueldo_mensual() {
    ...
    while (fscanf(archivo, "%s %lf %d %d %d %d %d %d %d\n", empleado.nombreCompleto, &empleado.salarioMensualUSD, &empleado.tiempoDescansoMinutos,
        ...
        int horas_extras = empleado.horasExtrasDiurnas; // Obtener el número de horas extras diurnas
...
// Función para calcular el número de horas trabajadas en el mes
int calcular_horas_trabajadas(int horaInicio, int minutoInicio, int horaFin, int minutoFin, int tiempoDescanso) {
    ...
    if (minutos_trabajados < 0) { // Si los minutos trabajados son negativos
        minutos_trabajados += 24 * 60; // Sumar un día completo en minutos
    }
...
double calcular_valor_hora(double salarioMensual, int tiempoDescanso) {
    int horas_laborables = HORAS_SEMANALES * 4 - (tiempoDescanso / 60 * 5); // Calcular el número de horas laborables en el mes
    return salarioMensual / horas_laborables; // Dividir el salario mensual entre las horas laborables y retornar el resultado
}

// Función para calcular el recargo por trabajo en feriado
...
// Función para calcular el recargo por horas extras
double calcular_recargo_horas_extras(double valorHora, int horasExtras) {
    return horasExtras * valorHora * RECARGO_EXTRA_DIURNO; // Multiplicar el número de horas extras por el valor de la hora y el porcentaje de recargo y retornar el resultado
}

// Función para calcular el aporte al IESS
...
// Función para validar que un valor entero esté dentro de un rango
void validar_entrada(int *valor, int min, int max, char *mensaje) {
    while (*valor < min || *valor > max) { // Mientras el valor sea menor que el mínimo o mayor que el máximo
